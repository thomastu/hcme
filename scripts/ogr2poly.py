...
python ogr2poly.py /path/to/shapefile.shp > /path/to/output.poly
"""
#!/usr/bin/env python
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-p", "--prefix", dest="outPrefix", help="Text to prepend to POLY filenames."
    )
    parser.add_option(
        "-b",
        "--buffer-distance",
    ...
    parser.add_option(
        "-f", "--field-name", dest="fieldName", help="Field name to use to name files."
    )
    parser.add_option(
        "-v",
        "--verbose",
